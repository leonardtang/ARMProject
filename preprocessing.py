...
class DataProcessing:
    ...
    def gen_train(self, seq_len):
        ...
        for i in range((len(self.stock_train)//seq_len)*seq_len - seq_len - 1):
            x = np.array(self.stock_train.iloc[i: i + seq_len, 2])
            y = np.array([self.stock_train.iloc[i + seq_len, 2]], np.float64)
    ...
    def gen_test(self, seq_len):
        ...
        for i in range((len(self.stock_test)//seq_len)*seq_len - seq_len - 1):
            x = np.array(self.stock_test.iloc[i: i + seq_len, 2])
            y = np.array([self.stock_test.iloc[i + seq_len, 2]], np.float64)
